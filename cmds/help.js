...
exports.run = (bot, message, args) => {
  ...
  const embed = new Discord.RichEmbed() // Define a new embed, if you are on the `stable` branch it will be new Discord.RichEmbed()
    ...
    .setTitle("Bots Command List")
    .setFooter(`Page ${page} of ${pages.length} • Bots | Beta Help`) // This is the default value, showing the default page and the amount of pages in the array.
  ...
  message.channel.send(embed).then(msg => { // Now, we will send the embed and pass the new msg object
	...
	embed.setTitle("Bots Command List")
        embed.setDescription(pages[page-1]); // Just like setting the first one, reset the Description to the new page
        embed.setFooter(`Page ${page} of ${pages.length} • Miyuki | Beta Help`); // This also sets the footer to view the current pagenumber
	...
	embed.setTitle("Bots Command List")
        embed.setDescription(pages[page-1]); // Just like setting the first one, reset the Description to the new page
        embed.setFooter(`Page ${page} of ${pages.length} • Bots | Beta Help`); // This also sets the footer to view the current pagenumber
        msg.edit(embed) // Then, we can push the edit to the message
      })
